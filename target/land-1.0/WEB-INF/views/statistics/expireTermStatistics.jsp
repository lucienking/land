...
<div id= "expireTerm_Container">
<div style="margin-top:10px;margin-left:50px;margin-bottom:20px;">
	选择农场：<input id="search_expireTerm_farm" style="width:120px;" />
</div>
<div  style="width:93%;text-align:center;margin-left:20px;">
	<table id="expireTerm_Datagrid" style="width:100%;"></table>
</div>
<div class="echarts_container">
<div id="expireTerm_people_main" class="echarts_main" _echarts_instance_="14365133674850444"
...
<div id="expireTerm_area_main" class="echarts_main" _echarts_instance_="14365133674851444"
	style="-webkit-tap-highlight-color: transparent; -webkit-user-select: none;
...
$("#search_expireTerm_farm").combobox({
    ...
    onLoadSuccess:function(){
		$("#search_expireTerm_farm").combobox("select","${farmCode}");
		if("${farmCode}" == "1") {
			$(this).combobox("readonly",false); 
			initExpireTermPie("","海南农垦");
		}
	},
	onSelect:function(rec){
		initExpireTermPie(rec.code=="1"?"":rec.code,rec.name );
	}
}); 

function initExpireTermPie(farmCode,farmName){
	var peopleChart = echarts.init(document.getElementById('expireTerm_people_main')); 
	var areaChart = echarts.init(document.getElementById('expireTerm_area_main')); 
	$.ajax({
		url:"${ctx}/statistics/getExpireTermData",
		type:'GET',
		...
		success:function(data){
			...
			for(var o in data){
				peopleNames[o] = data[o].expireTerm;
				areaNames[o] = data[o].expireTerm;
				var m = {name: data[o].expireTerm,value:data[o].area};
				peopleData[o] = m;
				
				var m2 ={name: data[o].expireTerm,value:data[o].countNum};
				areaData[o] = m2;
			}
			$('#expireTerm_Datagrid').datagrid({
				title:"<div style='width:100%;text-align:center'>"+farmName+" 合同到期统计</div>",
			    data:data,
			    columns:[[
			        {field:'expireTerm',title:'到期范围',width:'16%',align:'center'},
			        {field:'area',title:'面积数(亩)',width:'10%'},
			    ...
			    onLoadSuccess:function(data){
			    	addSumCells('expireTerm_Datagrid','area','areaPer','countNum','countNumPer');
			    } 
			});
			var peopleOption = {
					title : {
