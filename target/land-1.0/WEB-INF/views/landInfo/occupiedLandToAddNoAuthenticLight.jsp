...
.occupiedLandToAddNoAuthenticLight_table {
	width: 65%;
	height: 40px;
	margin-left: 225px;
	margin-top: 10px;
...
<script type="text/javascript">
	$(function() {
		...
		/**
			input绑定回车事件 跳下一个
		 */
		var inputAry = $("#occupiedLandToAddNoAuthenticLight_search_form").find("input[type=text]");
		...
		$("#ssr_submit_btn").click(
				function() {
					...
					var formData = $('#occupiedLandToAddNoAuthenticLight_search_form').serializeArray();
					$.ajax({
						type : "POST",
						url : "${ctx}/occupiedLand/addOccupiedLandNoAuthenticLight",
						...
						success : function(data) {
							$.messager.alert('操作结果', "" + data.message + "",
									"info", function(r) {
										parent.$('#mainTabs').tabs('close', "农场未确权且被占用的纠纷土地信息登记表");
									});
						}
					});
				});
		...
		$("#num").change(function(n, v) {
			...
			if (n != null && n != "")
				$.ajax({
					type : "GET",
					url : "${ctx}/occupiedLand/getNoAuthFormByNum",
					data : {
						"num" : $("#num").val()
					},
					asyn : false,
...
<div id="container" style="width: 99%; height: 99%;">
	<form id="occupiedLandToAddNoAuthenticLight_search_form">
		<table id="occupiedLandToAddNoAuthenticLight_table" class="occupiedLandToAddNoAuthenticLight_table" border="1" aligen="center"
			cellspacing="0">
			<tr>
			...
			<tr>
				<td width="100%" height="20px" colspan="6" align="center"
					style="border: none; font-size: 23px; font-weight: bold;">农场未确权且被占用的纠纷土地信息登记表</td>
			</tr>
			<tr>
				<td height="12px" colspan="3"
			...
			<tr>
				<td width="15%" height="10px" colspan="1" rowspan="7" align="center"
			...
			<tr>
				<td height="12px" colspan="42"
					...
					&nbsp;&nbsp;<b>填表说明：</b>
					<p style = "margin:10px;">一、占地主体，填写数字：1、周边农村个体；2、村委会、村民集体；3、乡镇；4、其他。</p>
					<p style = "margin:10px;">二、占地形式，填写数字：1、自用；2、占地发包；3、强占不准农场使用；4、其他方式。</p>
					<p style = "margin:10px;">三、作过何种处理，填写数字：1、农场出面制止未果；2、农场上报政府，等待处理；3、政府已作过处理决定，但未确实收回；4、法院以做出判决，但未执行；5、其他处理方式。</p>
					<p style = "margin:10px;">四、林地是市县土地利用总体规划中，确定为林业用途的土地。非林地，除规划为林地以外的其他用途土地。</p>
					<p style = "margin:10px;">五、如果是个人占用，填写占用人姓名及占用人证件号码。
				</td>
			</tr>
			<tr>
