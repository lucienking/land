...
<div id= "contractArea_Container">
<div style="margin-top:10px;margin-left:50px;margin-bottom:20px;">
	选择农场：<input id="search_contractArea_farm" style="width:120px;" />
</div>
<div  style="width:93%;text-align:center;margin-left:20px;">
	<table id="contractArea_Datagrid" style="width:100%;"></table>
</div>
<div class="echarts_container">
<div id="contractArea_people_main" class="echarts_main" _echarts_instance_="14365133674850222"
...
<div id="contractArea_area_main" class="echarts_main" _echarts_instance_="14365133674851222"
	style="-webkit-tap-highlight-color: transparent; -webkit-user-select: none;
...
$("#search_contractArea_farm").combobox({
	...
	onLoadSuccess:function(){
		$("#search_contractArea_farm").combobox("select","${farmCode}");
		if("${farmCode}" == "1") {
			$(this).combobox("readonly",false); 
			initContractAreaPie("","海南农垦");
		}
	},
	onSelect:function(rec){
		initContractAreaPie(rec.code=="1"?"":rec.code,rec.name);
	}
}); 

function initContractAreaPie(farmCode,farmName){
	var peopleChart = echarts.init(document.getElementById('contractArea_people_main')); 
	var areaChart = echarts.init(document.getElementById('contractArea_area_main')); 
	$.ajax({
		url:"${ctx}/statistics/getContractAreaData",
		type:'GET',
		...
		success:function(data){
			...
			for(var o in data){
				...
				var m = {name: data[o].areaArea,value:data[o].area};
				peopleData[o] = m;
				
				var m2 ={name: data[o].areaArea,value:data[o].countNum};
				areaData[o] = m2;
			}
			$('#contractArea_Datagrid').datagrid({
				title:"<div style='width:100%;text-align:center'>"+farmName+" 承包面积统计</div>",
			    data:data,
			    columns:[[
			    ...
			    onLoadSuccess:function(data){
			    	addSumCells('contractArea_Datagrid','area','areaPer','countNum','countNumPer');
			    }
			});
			var peopleOption = {
					title : {
