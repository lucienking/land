...
<form id="application" method="post">
	...
	<div id='bb' style="display:block;margin:10px auto;width:320px;">
			<div style="display:inline;padding-right:10px;">
				<a id="unregister" href="javascript:unregister()" class="easyui-linkbutton" style="width:90px;height:30px;" data-options="iconCls:'icon-undo'">注销</a>
			</div>
...
// 显示项目列表内容
$('#prjList').datagrid({
	url:'${ctx}/projectManage/listPassPrj',
	method:'get',
    rownumbers: true,
    pagination: true,
    pageList:[10,15,20],
...
function prjDataDialog(title,selected){
    ...
    if(selected.prjStatus=='5'){
		$('#unregister').linkbutton('disable');		
	}
}
...
// 	注销
	function unregister(){
		$.messager.confirm('注销', '确认注销该项目？', function(r){
			var selected=$('#prjList').datagrid('getSelected');
			$.ajax({
				url:"${ctx}/projectManage/unregister",
				type:'POST',
				async:true,
				data:{'id':selected.id},
				success:function(data){
